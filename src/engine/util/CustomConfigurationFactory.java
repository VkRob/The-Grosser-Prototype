...
public class CustomConfigurationFactory extends ConfigurationFactory {
	...
	private static Configuration createConfiguration(final String name,
		...
		/*
		 * internal log4j2 messages will only be displayed if they are WARN or
		 * higher
		 */
		...
		appenderBuilder.add(builder.newLayout("PatternLayout")
				/* boring pattern */
				// .addAttribute("pattern", "[%d{HH:mm:ss}] (%t) [%c{1}] %level:
				// %m%n%throwable"));
				/* colorful pattern! */
				.addAttribute("pattern",
						"%style{[%d{HH:mm:ss}]}{white} %style{(%t)}{magenta} %style{[%c{1}]}{blue} %highlight{%level{length=5}: %m%n%throwable}"));
