...
// TODO remove SIZE variable and make the grosser.prototype.map dynamic and infinite

public class Map {

	// currently, SIZE determines the size of the grosser.prototype.map, but eventually it should
...
//	private ArrayList<Tile> tiles;
	...
	/**
	 * Creates the grosser.prototype.map object and automatically generates a new factory
	 * surrounded by grass
	 */
	public Map() {
		tiles = new HashMap<>();
	...
	private void generateNewWorld() {
		...
		// Index keeps track of which tile is at (i, j) on the grosser.prototype.map...
		// (Kind of a messy way of doing it)

		// Loop through the grosser.prototype.map, i = x, j = y, tile[i][j] == tiles.get(index)
		for (int x = -SIZE / 2; x < SIZE / 2; x++) {
			for (int y = -SIZE / 2; y < SIZE / 2; y++) {
				...
				if (x <= r1Width && y <= r1Height && x >= 0 && y >= 0) {
					...
					if ((x == r1Width || y == r1Height && (x >= r2Width || y >= r2Height)) || (x == 0 || y == 0))
	...
	public HashMap<TilePosition, Tile> getTiles() {
		return tiles;
	}
	...
	public ArrayList<Tile> getTilesTouching(Rectangle box) {
		...
		for (TilePosition tPos : tiles.keySet()) {
			Rectangle tBox = new Rectangle(tPos.getX() * Tile.SIZE, tPos.getY() * Tile.SIZE, Tile.SIZE, Tile.SIZE);
    ...
    public Tile getTile(int x, int y) {
        return tiles.get(new TilePosition(x, y));
    }
//    public Tile getTile(int x, int y) {
